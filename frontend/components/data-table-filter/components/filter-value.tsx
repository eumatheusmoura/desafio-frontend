...
) as typeof FilterValueComponent;

function FilterValueComponent<TData, TType extends ColumnDataType>({
  filter,
  column,
  actions,
  strategy,
...
export const FilterValueController = memo(
  FilterValueControllerComponent
) as typeof FilterValueControllerComponent;

function FilterValueControllerComponent<TData, TType extends ColumnDataType>({
  filter,
  column,
  actions,
  strategy,
...
export function FilterValueNumberController<TData>({
  ...
  const changeNumber = useCallback(
    (value: number[]) => {
      ...
      const type = isNumberRange ? "range" : "single";

      if (type === "single") {
        newValues = [value[0]];
      } else if (minMax) {
        newValues =
          value[0] - minMax[0] < minMax[1] - value[0]
            ? createNumberRange([value[0], minMax[1]])
            : createNumberRange([minMax[0], value[0]]);
      } else {
        newValues = [value[0], value[0]];
      }
