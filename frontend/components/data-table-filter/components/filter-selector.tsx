...
) as typeof FilterSelectorComponent;
  ...
  const exportToCSV = useCallback(() => {
    ...
    try {
      ...
      const dataToExport = hasSelection
        ...
        : data;
      const exportType = hasSelection ? "selecionados" : "todos";
    ...
    >
      <PopoverTrigger asChild>
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            className={cn("h-8 text-stone-500", hasFilters && "w-fit !px-2")}
          >
            <FilterIcon className="size-4" />
            {!hasFilters && <span>Filtrar</span>}
          </Button>
        </div>
      </PopoverTrigger>
      <PopoverContent
